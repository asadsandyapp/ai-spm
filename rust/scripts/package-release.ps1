<#
.SYNOPSIS
  Builds a production release and packages it into dist\windows\ as a
  versioned zip (the .msi installer + INSTALL-NOTES.txt) - the standing
  convention so a "give me the current build" request always produces the
  same self-contained deliverable in the same place. dist\linux\ is the
  Linux counterpart, produced by scripts/package-release-linux.sh.

.DESCRIPTION
  1. cargo build --release --workspace
  2. cargo wix -p agent   (produces target\wix\agent-<version>-x86_64.msi)
  3. Copies the .msi and packaging\INSTALL-NOTES.txt into dist\windows\
  4. Zips them into dist\windows\AI-SPM-DLP-Agent-<version>.zip
...
Write-Host "== Assembling dist\windows\ (version $version) ==" -ForegroundColor Cyan
